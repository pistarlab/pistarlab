...
def install():
    ...
    for spec in get_component_specs():
        ctx.register_component_spec(**spec, extension_id=EXTENSION_ID, extension_version=EXTENSION_VERSION)

    for spec in get_task_specs():
        ctx.register_task_spec(**spec, extension_id=EXTENSION_ID, extension_version=EXTENSION_VERSION)
...
def create_spec_list_file():
    """
    Generate Extension Meta data file
    """
    import sys
    sys.path.append("../")
    import json
    with open("{}_v{}.json".format(EXTENSION_ID, EXTENSION_VERSION), 'w') as f:
...
if __name__ == "__main__":
    """
    Instructions: From the extension root package directory run: 
        python -m  pistarlab_rllib.extension
    """

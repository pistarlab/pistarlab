...
function getImageIdFromUID(uid) {
  let id = Math.abs(hashCode(uid));
  return id % 60;
}

function getImageIdFromUIDOld(uid) {
  let id = parseInt(uid.split("-")[1]);
  return id % 60;
}

export { getImageIdFromUID, timedelta, timedeltafordate, timepretty, uidsort, timelength, formatNum, numberToString };
